...
- `src/exec/emu.rs` - Updated to use `lp_riscv_emu::{EmulatorError, trap_code_to_string}`, `lp_riscv_inst::format_instruction`
- `src/backend/codegen/emu.rs` - Updated to use `lp_riscv_inst::Gpr`, `lp_riscv_emu::*`, `lp_riscv_elf::load_elf`
- `src/backend/codegen/builtins_linker.rs` - Updated to use `lp_riscv_elf::{ElfLoadInfo, load_elf, load_object_file}`
