...
/// Set a custom writer function for USB serial output (JTAG-Serial only)
///
/// This allows routing esp-println output through your own USB serial instance
/// instead of using direct register access. The function will be called with
/// byte slices that should be written to USB serial.
///
/// # Safety
/// The function pointer must remain valid for the lifetime of the program.
/// This should be called once at startup before any println! calls.
///
/// # Example
/// ```no_run
/// unsafe {
///     esp_println::set_custom_writer(|bytes| {
///         // Write bytes to your USB serial instance
///         your_serial_instance.write(bytes).ok();
///     });
/// }
/// ```
#[cfg(all(
...
pub unsafe fn set_custom_writer(writer: serial_jtag_printer::CustomWriterFn) {
    serial_jtag_printer::set_custom_writer(writer);
}
...
mod serial_jtag_printer {
    ...
    /// Custom writer function pointer
    /// If set, this function will be called instead of direct register access
    pub type CustomWriterFn = fn(&[u8]);
    static CUSTOM_WRITER: AtomicPtr<()> = AtomicPtr::new(core::ptr::null_mut());

    /// Set a custom writer function for USB serial output
    ///
    /// This allows routing esp-println output through your own USB serial instance
    /// instead of using direct register access. The function will be called with
    /// byte slices that should be written to USB serial.
    ...
    /// The function pointer must remain valid for the lifetime of the program.
    /// This should be called once at startup before any println! calls.
    pub unsafe fn set_custom_writer(writer: CustomWriterFn) {
        CUSTOM_WRITER.store(writer as *mut (), Ordering::Release);
    }
    ...
    impl super::Printer {
        pub fn write_bytes_assume_cs(&mut self, bytes: &[u8]) {
            // Check if a custom writer is set
            let custom_writer_ptr = CUSTOM_WRITER.load(Ordering::Acquire);
            if !custom_writer_ptr.is_null() {
                // Use custom writer
                let custom_writer: CustomWriterFn =
                    unsafe { core::mem::transmute(custom_writer_ptr) };
                custom_writer(bytes);
                return;
            }
        ...
        pub fn flush(&mut self) {
            // Check if a custom writer is set - if so, flush is handled by the writer
            let custom_writer_ptr = CUSTOM_WRITER.load(Ordering::Acquire);
            if custom_writer_ptr.is_null() {
                // Only flush if using direct register access
                fifo_flush();
            }
        }
    }
