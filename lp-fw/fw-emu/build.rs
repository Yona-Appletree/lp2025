fn main() {
    ...
    // We need to go up to fw-emu/, then to lp-fw/, then to root/, then to lp-glsl/, then to crates/, then to lp-emu-guest
    let manifest_dir = std::env::var("CARGO_MANIFEST_DIR").expect("CARGO_MANIFEST_DIR not set");

    // Go from lp-fw/fw-emu to lp-glsl/crates/lp-emu-guest
